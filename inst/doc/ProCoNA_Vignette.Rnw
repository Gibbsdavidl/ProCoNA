...
options(keep.source = TRUE, width = 70, stringsAsFactors=F, digits=2)
foo <- packageDescription("ProCoNA")
x <- capture.output(library(ProCoNA))
@

\title{ProCoNA: Protein Co-expression Network Analysis (Version \Sexpr{foo$Version})}
...
In order to use this package, the data needed to be in a matrix form, with peptides (or proteins) in columns, and samples in rows. However, most MS data is not in this format. What to do!? One solution is to ues the MSnbase package in Bioconductor, we can read data, normalize it, summarize it by feature, and export a quantitative expression! Please note that ProCoNA requires input matrices to have samples in rows, which means the result of MSnbase::exprs must be transposed before use. Be sure to see RforProteomics for more information.

<<Working from raw data.>>=
library(MSnbase)
file <- dir(system.file(package = "MSnbase", dir = "extdata"), full.names = TRUE, pattern = "mzXML$")
...
peptideData <- t(exprs(qnt2))

# or you can pass the MSnbase object directly to the buildProconaNetwork method!
# buildProconaNetwork(qnt2, ...) 
